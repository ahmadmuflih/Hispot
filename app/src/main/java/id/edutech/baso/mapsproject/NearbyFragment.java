...
public class NearbyFragment extends Fragment{
    ...
    public void initNearby(){
        ...
        Call<PostResults> postResultsCall = APIService.service.getNearby("","",api_key);

        /*---- Offline Mode */

        RealmResults<Post> capture = realm.where(Post.class).equalTo("statusNearby","1").findAll();
        /*
        ...
        postResultsCall.enqueue(new Callback<PostResults>() {
            ...
            public void onResponse(Call<PostResults> call, Response<PostResults> response) {
                if(response.isSuccessful()){
                    /*      Update Status Nearby */

                    realm.beginTransaction();
                    for(Post post: posts){
                        post.setStatusNearby("");
                    ...
                    Preferences.setIntPreferences("vrnearbyid",locationID,getActivity());
                    Preferences.setStringPreferences("vrnearbyurl",locationVR,getActivity());
            ...
            public void onFailure(Call<PostResults> call, Throwable t) {
                ...
                locationVR = Preferences.getStringPreferences("vrnearbyurl",getActivity());
                locationID = Preferences.getIntPreferences("vrnearbyid",getActivity());
                handleIntent(getActivity().getIntent());
            }
        });
